	...
	public void run() {
		...
		while( canRun ) {
			try {
				...
				System.out.println("CUSTOMER: requesting resources");
				if( theBank.requestResources( customerNum, request ) ) {
					System.out.println("CUSTOMER: request approved");
					// Use the resources
					SleepUtilities.nap();

					// Release the resources
					System.out.println("CUSTOMER: releasing resources");
					theBank.releaseResources( customerNum, request );
				}
				else {
					System.out.println("CUSTOMER: request denied");
				}
			}
